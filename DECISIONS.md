...
### 放弃方案 5: 只固定读取内置样例数据
- **原因：** 如果只能读取内置样例，面试官很难确认分类和合并逻辑不是针对样例结果写死的
- **选择方案：** 保留文件上传接口，允许替换 JSONL 日志后重新跑完整分析流程
...
每次启动或上传新日志时，系统会先清空 SQLite 中的 `logs` 和 `issues`，再重新执行解析、标准化、分类、合并和评分流程。

这样设计的原因是：当前项目是批处理 MVP，如果不清空旧数据，样例日志变更或 fingerprint 规则调整后，旧 issue 可能继续残留在数据库里，导致统计结果和当前输入不一致。线上系统可以改为按批次 ID 或时间窗口保存历史结果；本项目为了可验证性，选择每次处理都生成一份干净结果。
